...
export default class TitleScene extends Phaser.Scene {
    ...
    create(): void {
        const startButton = this.add
            .rectangle(window.innerWidth / 2, 780, 250, 80, 0x0000ff)
            .setInteractive();

        startButton.setDepth(5);

        const text = this.add
            .text(window.innerWidth / 2, 780, "Start Game", {
                fontSize: "32px",
            })
            .setOrigin(0.5);
        text.setDepth(5);
        ...
        const loreBox = this.add
            .rectangle(window.innerWidth / 2, 500, 600, 700, 0xcccccc)
            .setInteractive();

        this.add.text(window.innerWidth / 2 - 100, 200, "DISCO TOWER", {
            fontSize: "32px",
            color: "#000",
        });
